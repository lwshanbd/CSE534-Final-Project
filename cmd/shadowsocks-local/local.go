...
func parseServerConfig(config *ss.Config) {
	...
	if len(config.ServerPassword) == 0 {
		...
		n := len(srvArr)
		servers.srvs = make([]*Server, n)

		for i, s := range srvArr {
			if hasPort(s) {
				log.Println("ignore server_port option for server", s)
				servers.srvs[i] = &Server{s, config.Method, config.Password}
			} else {
				servers.srvs[i] = &Server{net.JoinHostPort(s, srvPort), config.Method, config.Password}
			}
		}
	} else {
		// multiple servers
		...
		for _, serverInfo := range config.ServerPassword {
			...
			servers.srvs[i] = &Server{server, encmethod, passwd}
			i++
		}
	}
	servers.failCnt = make([]int, len(servers.srvs))
	for _, se := range servers.srvs {
...
func connectToServer(serverId int, rawaddr []byte, addr string) (remote *ss.Conn, err error) {
	se := servers.srvs[serverId]
	// construct a fresh AEAD cipher
	aeadCiph, err := ss.NewCipher(se.encMethod, se.password)
	if err != nil {
		log.Println("error creating cipher:", err)
		return nil, err
	}
	remote, err = ss.DialWithRawAddr(rawaddr, se.server, aeadCiph)
	if err != nil {
